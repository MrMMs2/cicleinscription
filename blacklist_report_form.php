 ...
 */
class mod_cicleinscription_blacklist_report_form extends moodleform {
	...
	public function definition() {
		...
		// Adicionando campo tipo de entrada
		$mform->addElement('select', 'statusblacklist', get_string('inputtype','cicleinscription'), array('s'=>'Ativo', 'n' => 'Inativo'), array('style'=>'width: 41%'));

		// Add button
        $mform->addElement('submit', 'addfilter', get_string('addfilter','filters'));
        $coursemodulesid = optional_param('coursemodulesid', 0, PARAM_INT);
        $removefilter = get_string('removeall','filters');
        $mform->addElement('html', "<a href='{$CFG->wwwroot}/mod/cicleinscription/blacklist_report.php?coursemodulesid={$coursemodulesid}&page=0&perpage=20'>{$removefilter}</a>");
	}
}